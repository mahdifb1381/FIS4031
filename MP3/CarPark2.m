...
fis = mamfis('Name', Fisname, ...
             'AndMethod', Andmethod, ...
             'OrMethod', Ormethod, ...
             'ImplicationMethod', Impmethod, ...
             'AggregationMethod', Aggmethod, ...
             'DefuzzificationMethod', Defuzzmethod);

%% Add Variables
fis = addInput(fis, [0 20], 'Name', 'x');
fis = addInput(fis, [-90 270], 'Name', 'phi');
fis = addOutput(fis, [-40 40], 'Name', 'theta');

%% Add Membership Function
fis = addMF(fis, 'x', 'trapmf', S2_x, 'Name', 'S2');
fis = addMF(fis, 'x', 'trimf', S1_x, 'Name', 'S1');
fis = addMF(fis, 'x', 'trimf', CE_x, 'Name', 'CE');
fis = addMF(fis, 'x', 'trimf', B1_x, 'Name', 'B1');
fis = addMF(fis, 'x', 'trapmf', B2_x, 'Name', 'B2');

fis = addMF(fis, 'phi', 'trimf', S3_phi, 'Name', 'S3');
fis = addMF(fis, 'phi', 'trimf', S2_phi, 'Name', 'S2');
fis = addMF(fis, 'phi', 'trimf', S1_phi, 'Name', 'S1');
fis = addMF(fis, 'phi', 'trimf', CE_phi, 'Name', 'CE');
fis = addMF(fis, 'phi', 'trimf', B1_phi, 'Name', 'B1');
fis = addMF(fis, 'phi', 'trimf', B2_phi, 'Name', 'B2');
fis = addMF(fis, 'phi', 'trimf', B3_phi, 'Name', 'B3');

fis = addMF(fis, 'theta', 'trimf', S3_theta, 'Name', 'S3');
fis = addMF(fis, 'theta', 'trimf', S2_theta, 'Name', 'S2');
fis = addMF(fis, 'theta', 'trimf', S1_theta, 'Name', 'S1');
fis = addMF(fis, 'theta', 'trimf', CE_theta, 'Name', 'CE');
fis = addMF(fis, 'theta', 'trimf', B1_theta, 'Name', 'B1');
fis = addMF(fis, 'theta', 'trimf', B2_theta, 'Name', 'B2');
fis = addMF(fis, 'theta', 'trimf', B3_theta, 'Name', 'B3');
